...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
   ...
   useEffect(() => {
      const fetchData = async () => {
         try {
            const response = await axiosWithAuth(token).get(`${apiUrl}/api/${backendName}/users`);
            setUsers((response.data as User[]).reverse());
         } catch (error) {
   ...
   const createUser = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
         const response = await axiosWithAuth(token).post(`${apiUrl}/api/${backendName}/users`, newUser);
   ...
   const handleUpdateUser = async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      try {
         await axiosWithAuth(token).put(`${apiUrl}/api/${backendName}/users/${updateUser.id}`, {
   ...
   const deleteUser = async (userId: number) => {
      try {
         await axiosWithAuth(token).delete(`${apiUrl}/api/${backendName}/users/${userId}`);
